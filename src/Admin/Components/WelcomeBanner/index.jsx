...
const WelcomeBanner = ({ idNumber }) => {
    ...
    const handleClickLocal = () => {
        localStorage.setItem("hideLocal", "true")
        history.push(`/adminDashboard/${idNumber}`)
    }

    const handleClickSession = () => {
        sessionStorage.setItem("hideSession", "true")
        history.push(`/adminDashboard/${idNumber}`)
    }

    return (
        <Container>
            {localStorage.getItem("hideLocal") || sessionStorage.getItem("hideSession") === "true" ? null :
                <BannerDiv>
                    <LeftDiv>
